...
const FirebaseDataFetcher = () => {
  ...
  const saveData = async () => {
    try {
      ...
      const formattedData = {
        ...formData,
        background_knowledge: `[${formatArrayToString(formData.background_knowledge.filter(item => item.trim() !== ""))}]`,
        implicit_message: `[${formatImplicitMessageToString(formData.implicit_message.filter(item => item.trim() !== ""))}]`
      };
